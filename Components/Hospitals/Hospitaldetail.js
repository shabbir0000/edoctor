...
const Hospitaldetail = ({ navigation, route }) => {
    ...
    useEffect(() => {
        ...
        const q = query(coll, where("phone", '==', phone), where("email", '==', email), where("fullname", '==', name), where("role", '==', "subadmin"));
                    ...
                    <>
                        ...
                        <ScrollView>
                            <View style={tw`w-90 h-40 mt-5 bg-white`}>
                                ...
                                <Text style={[tw`font-bold text-xl ml-3 `, { color: "#00B1E7" }]} >{data.selecteduser.fullname.toUpperCase()}DETAIL</Text>
                                <View style={tw`flex-row w-80 ml-3 h-10 items-center justify-between `}>
                                    <Image
                                ...
                                <View style={tw`flex-row ml-3 w-80 h-10 items-center justify-between `}>
                                    <Image
                                    ...
                                    >
                                        <Text numberOfLines={1} style={tw`w-70 font-normal underline`}>{data.selecteduser.phone}</Text>
                                    </TouchableOpacity>
                                </View>
                                ...
                                <View style={tw`flex-row ml-3 w-80 h-10 items-center justify-between `}>
                                    ...
                                    <Text numberOfLines={2} style={tw`w-70 font-normal`}>{data.selecteduser.address}</Text>
                            ...
                            <View style={tw`h-30  self-center  w-90 bg-white mt-5 `}>
                                <Text style={[tw`text-center font-bold text-lg`, { color: "#00B1E7" }]}>Hospital Open Days</Text>
                            ...
                            <View style={tw`h-70 w-90 justify-center bg-white mt-5 rounded-md`}>
                                <View style={tw` h-7 w-80  self-center flex-row justify-between items-center`}>
                                    <Text style={[tw`text-base font-bold`,{color:"#00B1E7"}]}>TOP DOCTORS</Text>
                                ...
                                <ScrollView showsHorizontalScrollIndicator={false} horizontal={true}>

                                    {
                                        GetData1.length ?
                                            ...
                                            :
                                            <View style={tw`self-center justify-center items-center w-90`}>
                                                <Text style={tw`text-center text-lg font-bold`}>NO DOCTORS RIGHT NOW</Text>
                                            </View>
                                    }
