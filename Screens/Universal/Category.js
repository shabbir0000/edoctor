  ...
  const getemergency = async () => {
    AsyncStorage.getItem("email").then((email) => {
      AsyncStorage.getItem("role").then((role) => {
        ...
        const q = query(coll, where('doctoremail', '==', email), where('todaydate', '==', showdate), where('status', '==', "pending"));
  ...
  const getacceptemergency = async () => {
    AsyncStorage.getItem("email").then((email) => {
      AsyncStorage.getItem("role").then((role) => {
        ...
        const coll = collection(db, 'Emergency');
        const q = query(coll, where('doctoremail', '==', email), where('todaydate', '==', showdate), where('status', '==', "accept"));
  ...
  const getrejectemergency = async () => {
    AsyncStorage.getItem("email").then((email) => {
      AsyncStorage.getItem("role").then((role) => {
        ...
        const coll = collection(db, 'Emergency');
        const q = query(coll, where('doctoremail', '==', email), where('todaydate', '==', showdate), where('status', '==', "reject"));
                      ...
                      {vrImages?.map((item, index) => (
                        <TouchableOpacity
                          ...
                          <View style={[tw`bg-${cat === item.text ? 'blue-400' : 'white'} h-10 w-30 ml-5 border flex-row items-center justify-evenly rounded-3xl`, { borderRadius: 50, borderColor: '#00B1E7' }]}>
                            ...
                            <Text numberOfLines={1} style={tw`text-center text-${cat === item.text ? "white" : "black"} w-25`}>{item.text}</Text>
                          </View>
                        </TouchableOpacity>
...

                  {
                    GetData1.map((data, index) => (
                      <>
                        ...
                        >
                          <View style={[tw`border flex-col justify-around   w-80 h-${data.selecteduser.status === "pending" ? "70" : "40"}  self-center mt-5`, { borderColor: "red" }]}>

                            <View style={tw`h-40  justify-evenly w-35 `}>
                              <Text numberOfLines={1} style={tw`font-bold ml-2 w-75 underline text-red-500 text-2xl`}>EMERGENCY</Text>

                              <View style={tw`flex-row ml-5 items-center justify-between w-70`}>
                                <Image
                                  source={require("../../Images/user.png")}
                                  style={tw`w-5 h-5`}

                                />
                                <Text numberOfLines={1} style={tw`font-light ml-2 w-75 text-xl`}>{data.selecteduser.username.toUpperCase()}</Text>
                              ...
                              <View style={tw`flex-row ml-5 items-center justify-between w-70`}>
                                ...
                                />
                                <Text numberOfLines={1} style={tw`font-light ml-2 w-75 text-sm`}>{data.selecteduser.phone}</Text>
                              ...
                              <View style={tw`flex-row ml-5 items-center justify-between w-70`}>
                                ...
                                />
                                <Text numberOfLines={1} style={tw`font-light ml-2 mt-1 w-75  text-sm`}>{data.selecteduser.city.toUpperCase()}</Text>
                              ...
                              <View style={tw`flex-row ml-5 items-center justify-between w-70`}>
                                ...
                                />
                                <Text numberOfLines={1} style={tw`font-light ml-2 mt-1 w-75  text-base`}>{data.selecteduser.status.toUpperCase()}</Text>
                                    ...
                                    loading1 && data.selecteduser.userid === setcatid ?
                                      ...
                                      <TouchableOpacity
                                        onPress={() => {
                                          ...
                                          updateemergencydoc('accept', data.selecteduser.userid)
                                        }}
                                      >
                                        <View style={tw`self-center rounded-md bg-green-500 w-70 items-center justify-center h-10  `}>
...

                                  {
                                    ...
                                    loading2 && data.selecteduser.userid === setcatid ?
                                      ...
                                      <TouchableOpacity
                                        onPress={() => {
                                          setcatid(data.selecteduser.userid)
                                          updateemergencydoc1('reject', data.selecteduser.userid)
                                        }}
                                      >
                                        <View style={tw`self-center rounded-md bg-red-500 w-70 items-center justify-center h-10  `}>
